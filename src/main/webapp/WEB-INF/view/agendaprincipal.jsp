...
<c:forEach items="${requestScope.lista}" var="c">
	<tr><td>${c.titulo}</td><td>${c.descricao}</td><td>${c.data_inicio}</td><td>${c.data_conclusao}</td><td>${c.status}</td><td><a href="/agendaPessoal/deleteTarefa?id_tarefa=${c.id}">Deletar Tarefa!</a></td></tr>
</c:forEach>
